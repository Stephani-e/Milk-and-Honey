...
export default function ProfilesManagement() {
    ...
    const handleSensitiveUpdate = async (e: React.SyntheticEvent<HTMLFormElement>) => {
        e.preventDefault();
        ...
        <div className="p-4 md:p-12 max-w-6xl mx-auto min-h-screen pb-24 relative">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
                <div className='w-full text-center md:text-left'>
                    <h1 className="text-3xl md:text-4xl font-serif font-bold text-brand-primary tracking-tight">Admin
                        Personnel</h1>
                    <p className="text-brand-secondary text-sm font-medium">Lagos Province 56 • Administration
                        Console</p>
                </div>
            ...
            <div className="bg-white border border-brand-accent rounded-[2.5rem] overflow-hidden shadow-sm">
                <div className='hidden md:block overflow-x-auto'>
                    <table className="w-full text-left whitespace-nowrap">
                        ...
                        <tr>
                            <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-brand-primary/40">Authorized
                                Identity
                            </th>
                            <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-brand-primary/40">Security
                                Level
                            </th>
                            <th className="px-8 py-5 text-right text-[10px] font-black uppercase tracking-widest text-brand-primary/40">Access
                                Controls
                            </th>
                        </tr>
                        </thead>
                    ...
                    {filteredProfiles.length === 0 && !loading && (
                        <div className="flex flex-col items-center justify-center py-24 px-8 text-center">
                            <div
                                className="h-15 w-15 bg-brand-surface rounded-[2rem] flex items-center justify-center text-brand-primary/20 mb-6 border border-brand-accent">
            ...
            {revokeTarget && (
                <div
                    className="fixed inset-0 bg-red-900/20 backdrop-blur-md z-[250] flex items-center justify-center p-4">
                    <div
                        className="bg-white p-8 rounded-[2.5rem] shadow-2xl border border-red-100 max-w-sm w-full animate-in zoom-in duration-200">
                        <div className="flex justify-between items-start mb-6">
                            <div
                                className="h-12 w-12 bg-red-50 rounded-2xl flex items-center justify-center text-red-600 mb-4">
                                <AlertCircle size={28}/>
                            </div>
                            <button
                        ...
                        <p className="text-sm text-gray-500 leading-relaxed mb-8">
                            You are about to revoke access for <strong
                            className="text-brand-primary">{revokeTarget.full_name}</strong>.
                            They will be moved to the <span
                            className="font-bold uppercase text-[10px] tracking-widest">Revoked</span> tab for 5 days
                            ...
                            {ROLE_HIERARCHY.filter(r => r !== choiceTarget.user.role).map(role => (
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <span
                                                className="block text-[9px] font-black uppercase text-gray-400 mb-1 tracking-widest">Assign As</span>
                                            <span
                                                className="block font-bold text-brand-primary text-lg capitalize">{role.replace('-', ' ')}</span>
                                        </div>
                                        <div
            ...
            {isCreateModalOpen && (
                <div
                    className="fixed inset-0 bg-brand-primary/40 backdrop-blur-md z-[200] flex items-center justify-center p-4">
                    <div
                        className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 border border-brand-accent animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-2xl font-serif font-bold text-brand-primary">Create Staff</h2>
                            <button onClick={() => setIsCreateModalOpen(false)}
                                    className="p-2 hover:bg-brand-surface rounded-full transition-colors"><X/></button>
                        </div>

                        <form onSubmit={handleCreateUser} className="space-y-5">
                            <div>
                                <label className="block text-[10px] font-black uppercase text-brand-primary mb-2 ml-1">Full
                                    Identity Name</label>
                                <input
                                    required
                            ...
                            <div>
                                <label className="block text-[10px] font-black uppercase text-brand-primary mb-2 ml-1">Email
                                    Address</label>
                                <input
                                    required
                                    type="email"
                            ...
                            <div>
                                <label className="block text-[10px] font-black uppercase text-brand-primary mb-2 ml-1">Initial
                                    Password</label>
                                <input
                                    required
                                    type="text"
                                ...
                                />
                                <p className="text-[10px] text-gray-400 mt-2 ml-1">You will need to securely share this
                                    password with them.</p>
                            </div>
                            <div>
                                <label className="block text-[10px] font-black uppercase text-brand-primary mb-2 ml-1">Access
                                    Level</label>
                                <select
                            ...
                            >
                                {isCreating ? <Loader2 className="animate-spin"/> : <><UserPlus size={18}/> Create
                                    Account</>}
                            </button>
                        </form>
                    </div>
                ...
                <div
                    ...
                    <div
                        ...
                        <form onSubmit={handleSensitiveUpdate} className="space-y-5">
                            ...
                            <div>
                                <label className="block text-[10px] font-black uppercase text-brand-primary mb-2 ml-1">New
                            ...
                            <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100 flex gap-3">
                                ...
                                <p className="text-[10px] text-amber-700 leading-tight">
                                    Updating these fields will immediately change how this user logs in. Ensure you
                                    communicate new credentials to them.
                                </p>
                            </div>
