        ...
        <div className="min-h-screen bg-brand-surface p-4 md:p-12 font-sans">
            <div className="max-w-5xl mx-auto">
                <Link href="/admin/gallery"
                      className="text-sm font-bold text-brand-secondary mb-6 block hover:underline">← Back to Gallery
                    Dashboard</Link>

                <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-brand-accent">
                    <h1 className="text-2xl md:text-3xl font-serif font-bold text-brand-primary mb-8">Edit Gallery
                        Entry</h1>
                    ...
                    <div className="space-y-8 md:space-y-10">
                            ...
                            <div className="space-y-4 md:space-y-6 animate-in fade-in slide-in-from-top-2">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 opacity-75">
                                    {["Sunday", "Tuesday", "Thursday"].map((day) => (
                                        <div key={day}
                                             className={`p-3 rounded-xl border text-center font-bold cursor-not-allowed ${weeklyType === day ? "bg-brand-primary text-white" : "bg-gray-50 text-gray-300 border-gray-100"}`}>{day}</div>
                                    ))}
                                </div>
                        ...
                        {category === "Special" && (
                            <div className="space-y-4 md:space-y-6 animate-in fade-in">
                                ...
                                {isMultiDay && <input placeholder="Day Identifier (e.g. Day 3)"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="relative">
                                        ...
                                        <input type="url" placeholder="https://instagram.com/..."
                                               ...
                                               onChange={(e) => setFormData({...formData, link_ig: e.target.value})}/>
                                    </div>
                                    <div className="relative">
                        ...
                        {mediaItems.length > 0 && (
                            <div className="space-y-4 animate-in slide-in-from-bottom-4">
                                <label
                                    className="text-[9px] md:text-xs font-bold uppercase tracking-widest text-purple-400 mb-2">Step
                                    5: Edit/Change Old Media & Preview New Media. Captions can be added to each.</label>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-6 pt-8 border-t border-gray-100">
                            ...
                            <div className='flex flex-col gap-2'>
                                <label
                                    className="text-[9px] md:text-xs font-bold uppercase tracking-widest text-purple-400">Step
                                    7: Publish Sermon or Save As Draft</label>
                                <div className="flex flex-col sm:flex-row gap-4">
                                    <button type="button" onClick={() => handleSubmit('draft')} disabled={loading}
                                            className="w-full sm:flex-1 py-4 border-2 border-brand-primary text-brand-primary rounded-2xl font-bold disabled:opacity-50 hover:bg-brand-primary/5 transition-all">Revert
                                        to Draft
                                    </button>
                                    <button type="button" onClick={() => handleSubmit('published')} disabled={loading}
                                            className="w-full sm:flex-[2] py-4 bg-brand-primary text-white rounded-2xl font-bold disabled:opacity-50 shadow-lg hover:bg-slate-800 transition-all">{loading ? "Updating..." : "Save Changes"}</button>
                                </div>
                            </div>
                        </div>
                    </div>
