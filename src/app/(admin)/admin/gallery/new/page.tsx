...
import { X, Film, LinkIcon } from "lucide-react";
                    ...
                    <div className="space-y-8 md:space-y-10">
                            ...
                            <div className="space-y-4 md:space-y-6 animate-in fade-in slide-in-from-top-2">
                                ...
                                {weeklyType === "Sunday" && (
                                    <div className="bg-slate-50 p-4 md:p-6 rounded-2xl space-y-4 md:space-y-6">
                                        <label className="text-[9px] md:text-xs font-bold uppercase tracking-widest text-purple-400 mb-2">Step 1.2: Fill Sunday Information</label>
                        ...
                        {category === "Special" && (
                            <div className="space-y-4 md:space-y-6 animate-in fade-in">
                                ...
                                <div className="flex flex-col sm:flex-row gap-4">
                                    <button type="button" onClick={() => setIsMultiDay(false)} className={`flex-1 p-3 rounded-lg border font-bold ${!isMultiDay ? "bg-brand-primary text-white" : "text-brand-primary"}`}>One-Day</button>
                                    <button type="button" onClick={() => setIsMultiDay(true)} className={`flex-1 p-3 rounded-lg border font-bold ${isMultiDay ? "bg-brand-primary text-white" : "text-brand-primary"}`}>Multi-Day</button>
                                </div>
                                {isMultiDay && (
                                    <input placeholder="Day Identifier (e.g. Day 3)" className="w-full p-3 border rounded-lg text-brand-primary" value={formData.day_identifier} onChange={(e) => setFormData({...formData, day_identifier: e.target.value})} />
                                )}
                            </div>
                        )}
                        ...
                        {(category !== "") && (
                            <div className='flex flex-col gap-2 mt-2 pt-6 border-t border-gray-100'>
                                <label className="text-[9px] md:text-xs font-bold uppercase tracking-widest text-purple-400 mb-2">Step 3: External Platform Links (Optional)</label>
                        ...
                        {(weeklyType || category === "Monthly" || category === "Special") && (
                            <div className='flex flex-col gap-2'>
                                <label className="text-[9px] md:text-xs font-bold uppercase tracking-widest text-purple-400 mb-2">Step 4: Upload All Media (Pictures & Videos)</label>
                                <div className="p-6 md:p-10 border-2 border-dashed border-brand-accent rounded-3xl bg-slate-50/50 text-center animate-in fade-in">
                                    ...
                                    <UploadButton
                                        ...
                                        onUploadError={(error) => { toast.error("Upload failed. Please try again."); }}
                                    />
                                </div>
                            </div>
                        )}
                        ...
                        {mediaItems.length > 0 && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in slide-in-from-bottom-4">
                                <label className="text-[9px] md:text-xs font-bold uppercase tracking-widest text-purple-400 mb-2 col-span-full">Step 5: Preview the Media. Captions can be added to each. Media can be deleted individually.</label>
                                        ...
                                        <div className="w-full sm:w-24 h-32 sm:h-24 rounded-lg overflow-hidden bg-slate-100 flex-shrink-0 relative">
                                            {item.type === 'image' ? (
                                                ...
                                                <div className="w-full h-full relative group/video">
                                                    <video className="w-full h-full object-cover bg-slate-900" muted playsInline preload="metadata" onMouseOver={(e) => e.currentTarget.play().catch(() => {})} onMouseOut={(e) => { e.currentTarget.pause(); e.currentTarget.currentTime = 0; }}>
                                            ...
                                            <button type="button" onClick={() => removeItem(index)} className="absolute top-2 right-2 sm:top-1 sm:right-1 p-2 sm:p-1 bg-red-500 text-white rounded-full sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"><X size={12} className="sm:w-[10px] sm:h-[10px]" /></button>
                                        </div>
                                        <div className="flex-1 w-full">
                                            <label className="text-[9px] font-black text-gray-400 uppercase block mb-1">Caption</label>
                                            <textarea value={item.caption} onChange={(e) => updateCaption(index, e.target.value)} className="w-full text-brand-primary p-2 text-xs border rounded-lg h-20 sm:h-16 resize-none focus:ring-1 focus:ring-brand-primary outline-none" placeholder="Add context..." />
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        {(weeklyType || category === "Monthly" || category === "Special") && (
                            <div className="space-y-6 pt-8 border-t border-gray-100">
                                ...
                                <div className='flex flex-col gap-2'>
                                    <label className="text-[9px] md:text-xs font-bold uppercase tracking-widest text-purple-400">Step 7: Publish Gallery or Save As Draft</label>
                                    <div className="flex flex-col sm:flex-row gap-4">
                                        <button type="button" onClick={() => handleSubmit('draft')} className="w-full sm:flex-1 py-4 border-2 border-brand-primary text-brand-primary rounded-2xl font-bold">Save Draft</button>
                                        <button type="button" onClick={() => handleSubmit('published')} className="w-full sm:flex-[2] py-4 bg-brand-primary text-white rounded-2xl font-bold">Publish Gallery</button>
                                    </div>
                                </div>
                            </div>
                        )}
