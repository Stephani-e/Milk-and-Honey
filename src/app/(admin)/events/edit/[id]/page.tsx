                        ...
                        <div className="space-y-6">
                            ...
                            {eventType === 'recurring' && recurringDay === 'sunday' && title === 'Sunday Service' ? (
                                <div className="space-y-6">
                                    ...
                                    {/* THANKSGIVING FLYER */}
                                    {thanksgivingSession && (
                                        <div className="p-4 border-2 border-brand-primary border-dashed rounded-2xl bg-brand-primary/5">
                                            ...
                                            <div className="flex flex-col sm:flex-row gap-4 items-center">
                                                <div className="w-full sm:w-48 aspect-video bg-white border border-gray-200 rounded-xl flex items-center justify-center overflow-hidden flex-shrink-0">
                                                    {thanksgivingSession.flyer_url ? (
                                                        <img src={thanksgivingSession.flyer_url} alt="Thanksgiving" className="w-full h-full object-cover" />
                                                ...
                                                <div className="w-full">
                                                    {!thanksgivingSession.flyer_url ? (
                                                        <UploadButton
                                                            endpoint="imageUploader" appearance={{ button: "bg-brand-primary text-white text-[10px] px-4 py-2 rounded-lg w-full sm:w-auto" }} onClientUploadComplete={(res) => { setThanksgivingSession({...thanksgivingSession, flyer_url: res[0].url}); toast.success("Thanksgiving flyer uploaded!"); }}
                                                            onUploadError={(error) => { toast.error(`Upload Failed: ${error.message}`); }}
                                                        />
                                                    ) : <button type="button" onClick={() => setThanksgivingSession({...thanksgivingSession, flyer_url: ""})} className="bg-red-50 text-red-600 px-4 py-2 rounded-lg text-[10px] font-bold hover:bg-red-100 transition-colors">Remove Thanksgiving Flyer</button>}
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                            ...
                                            {standardSessions.map((session, index) => (
                                                <div key={index} className="p-4 border border-brand-accent rounded-2xl bg-slate-50">
                                                    ...
                                                    <div className="flex flex-col gap-4 items-center">
                                                        <div className="w-full aspect-video bg-white border border-gray-200 rounded-xl flex items-center justify-center overflow-hidden">
                                                            {session.flyer_url ? (
                                                        ...
                                                        <div className="w-full">
                                                            {!session.flyer_url ? (
                                                                <UploadButton
                                                                    endpoint="imageUploader" appearance={{ button: "bg-slate-800 text-white text-[10px] px-4 py-2 rounded-lg w-full" }} onClientUploadComplete={(res) => { updateSessionFlyer(index, res[0].url); toast.success(`${session.name} flyer uploaded!`); }}
                                                                    onUploadError={(error) => { toast.error(`Upload Failed: ${error.message}`); }}
                                                                />
                                                            ) : <button type="button" onClick={() => updateSessionFlyer(index, "")} className="w-full bg-red-50 text-red-600 px-4 py-2 rounded-lg text-[10px] font-bold hover:bg-red-100 transition-colors">Remove Flyer</button>}
                                                        </div>
                                                    </div>
                                                </div>
                                            ))}
                                ...
                                <div className="p-4 border border-brand-accent rounded-2xl bg-slate-50">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-6 items-start">
                                        <div className="w-full sm:w-48 aspect-square bg-white border-2 border-dashed border-gray-200 rounded-2xl flex items-center justify-center overflow-hidden">
                                        ...
                                        <div className="flex-1 w-full space-y-4">
                                            ...
                                            {!flyerUrl ? (
                                                <UploadButton endpoint="imageUploader" appearance={{ button: "bg-brand-primary text-white text-xs px-6 py-4 rounded-xl after:bg-brand-secondary w-full sm:w-auto" }} onClientUploadComplete={(res) => { setFlyerUrl(res[0].url); toast.success("Flyer uploaded!"); }}
                                                              onUploadError={(error) => { toast.error(`Upload Failed: ${error.message}`); }}
                                                />
                                    ...
                                    {standardSessions.length > 0 && title !== 'Sunday Service' && (
                                        <div className="mt-6 pt-6 border-t border-gray-200">
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                                {standardSessions.map((session, index) => (
                                                    <div key={index} className="bg-white border border-gray-200 p-2 rounded-lg text-xs flex justify-between">
                                                        <span className="font-medium text-gray-600">{session.name}</span>
                                                        <span className="text-brand-primary font-bold">{session.start_time}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
