...
import {
    Search, Calendar, Camera, PlayCircle, Loader2,
...
export default function GalleryPage() {
    ...
    // Search and Filter State (Identical to Sermons UX)
    ...
    return (
        <div className="flex flex-col bg-slate-50 min-h-screen pb-24 relative">
            ...
            {/* HERO SECTION */}
            <section className="relative py-24 md:py-32 bg-slate-900 overflow-hidden flex items-center justify-center min-h-[400px]">

                {/* IMAGE COLLAGE BACKGROUND */}
                <div className="absolute inset-0 grid grid-cols-2 md:grid-cols-4 grid-rows-2 gap-2 md:gap-3 p-2 md:p-3 opacity-30 scale-105">
                    {/* Image 1 (Tall - Spans 2 rows) */}
                    <div className="row-span-2 md:col-span-1 rounded-xl md:rounded-2xl overflow-hidden">
                        <img src="https://images.unsplash.com/photo-1438232992991-995b7058bbb3?q=80&w=800&auto=format&fit=crop" className="w-full h-full object-cover" alt="Gallery background 1" />
                    </div>
                    {/* Image 2 (Standard) */}
                    <div className="col-span-1 rounded-xl md:rounded-2xl overflow-hidden">
                        <img src="https://images.unsplash.com/photo-1510590337019-5ef8d3d32116?q=80&w=800&auto=format&fit=crop" className="w-full h-full object-cover" alt="Gallery background 2" />
                    </div>
                    ...
                    <div className="col-span-2 md:col-span-1 rounded-xl md:rounded-2xl overflow-hidden">
                        <img src="https://images.unsplash.com/photo-1478147424044-f252df6e8557?q=80&w=800&auto=format&fit=crop" className="w-full h-full object-cover" alt="Gallery background 3" />
                    </div>
                    ...
                    <div className="row-span-2 col-span-1 hidden md:block rounded-2xl overflow-hidden">
                        <img src="https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?q=80&w=800&auto=format&fit=crop" className="w-full h-full object-cover" alt="Gallery background 4" />
                    </div>
                    {/* Image 5 (Wide) */}
                    <div className="col-span-2 md:col-span-2 rounded-xl md:rounded-2xl overflow-hidden">
                        <img src="https://images.unsplash.com/photo-1529070538774-1843cb3265df?q=80&w=1200&auto=format&fit=crop" className="w-full h-full object-cover object-top" alt="Gallery background 5" />
                    </div>
                </div>
                ...
                <div className="max-w-7xl mx-auto px-4 md:px-6 relative z-10 text-center mt-4">
                    <span className="text-amber-400 font-bold tracking-[0.3em] uppercase text-xs md:text-sm mb-4 block drop-shadow-md">
                        Our Moments
                    </span>
            ...
            {loading ? (
                ...
                <>
                    ...
                    <section className="max-w-7xl mx-auto px-4 md:px-6 mb-10 w-full -mt-8 relative z-20">
                        <div className={`bg-white p-3 md:p-4 shadow-xl shadow-slate-200/50 border border-gray-100 rounded-xl flex flex-col relative transition-all duration-300 ${filterCategory === 'Sunday' || filterCategory === 'Monthly' || filterCategory === 'Special'}`}>
                            ...
                            <div className="flex flex-wrap items-center justify-end gap-2 md:gap-3 pt-4 mt-2 border-t border-gray-100 w-full">
                                ...
                                <div className="flex gap-2 flex-1 sm:flex-none justify-end">
                                    <select value={jumpMonth} onChange={(e) => setJumpMonth(e.target.value)} className="bg-slate-50 border border-gray-200 text-brand-primary text-[10px] md:text-xs font-bold rounded-lg px-2 md:px-3 py-1.5 outline-none focus:ring-1 focus:ring-brand-primary cursor-pointer w-1/2 sm:w-auto">
                                        <option value="All">All Months</option>
                                        {availableMonths.filter(m => m !== "All").map(m => <option key={m} value={m}>{m}</option>)}
                                    </select>
                                    <select value={jumpYear} onChange={(e) => setJumpYear(e.target.value)} className="bg-slate-50 border border-gray-200 text-brand-primary text-[10px] md:text-xs font-bold rounded-lg px-2 md:px-3 py-1.5 outline-none focus:ring-1 focus:ring-brand-primary cursor-pointer w-1/2 sm:w-auto">
                    ...
                    <section className="max-w-7xl mx-auto px-4 md:px-6 mb-24 w-full">
                        {Object.keys(groupedGalleries).length > 0 ? (
                                ...
                                {Object.keys(groupedGalleries).sort((a, b) => b.localeCompare(a)).map((year) => {
                                    ...
                                    return (
                                        <div key={year} className="flex flex-col w-full">
                                            {/* YEAR HEADER */}
                                            <button type="button" onClick={(e) => toggleYear(e, year)} className="flex items-center justify-between w-full text-left pb-2 mb-6 border-b-[3px] border-brand-primary/20 group hover:border-brand-primary transition-colors cursor-pointer">
                                                        ...
                                                        .map((month) => {
                                                            ...
                                                            return (
                                                                <div key={monthKey} className="flex flex-col bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-sm w-full overflow-hidden">
                                                                    <button type="button" onClick={(e) => toggleMonth(e, monthKey)} className="flex items-center justify-between w-full text-left pb-4 border-b border-gray-100 group cursor-pointer">
                                                                    ...
                                                                    {isMonthExpanded(monthKey) && (
                                                                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mt-6 animate-in slide-in-from-top-2 fade-in duration-300 w-full">
                                                                            {monthAlbums.map((album: any) => {
                                                                                ...
                                                                                return (
                                                                                    <Link key={album.id} href={`/gallery/${album.id}`} className="bg-slate-50 rounded-2xl md:rounded-3xl overflow-hidden border border-gray-100 group cursor-pointer hover:-translate-y-1 md:hover:-translate-y-2 transition-all duration-300 flex flex-col hover:shadow-lg w-full">

                                                                                        {/* Album Cover */}
                                                                                        <div className="h-48 relative overflow-hidden bg-slate-900 w-full">
                                                                                            ...
                                                                                            {cover?.type === 'video' ? (
                                                                                                <video src={cover.url} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 group-hover:scale-110 transition-transform duration-700" muted playsInline />
                                                                                            ) : (
                                                                                                <img src={cover?.url || "https://hegyctrfwn.ufs.sh/f/iMcVGeeTb1N4go9KLrcAQBW5E03lrCpOqKzJIRZUnG9sLDHa"} alt={album.title} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 group-hover:scale-110 transition-transform duration-700" />
                                                                                            )}

                                                                                            {/* Count Badge Overlay */}
                                                                                            <div className="absolute bottom-3 right-3 z-20 bg-black/60 backdrop-blur-md rounded-full px-3 py-1 flex items-center gap-1.5 text-white text-[10px] font-bold border border-white/20">
                                                                                                <Camera size={12} /> {fileCount}
                                                                                            </div>
                                                                                        </div>

                                                                                        <div className="p-4 md:p-5 flex flex-col flex-grow w-full overflow-hidden">
                                                                                            <div className="flex flex-wrap gap-1.5 mb-2">
                                                                                                <span className={`text-[8px] md:text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md shrink-0 ${album.service_category === "Weekly" ? "bg-purple-100 text-purple-700" : album.service_category === "Monthly" ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700"}`}>
                                                                                                    {getBadge(album)}
                                                                                                </span>
                                                                                            ...
                                                                                            <div className="mt-auto flex items-center justify-between text-[10px] md:text-xs text-gray-500 font-medium pt-2">
                                                                                                <span className="flex items-center gap-1"><Calendar size={12} className="text-gray-400 shrink-0"/> {new Date(album.service_date).toLocaleDateString('en-GB')}</span>
                                                                                                <span className="text-amber-600 font-bold group-hover:underline">Open Album</span>
                                                                                            </div>
                                                                                        </div>
                                                                                    </Link>
                                                                                );
                            ...
                            <div className="py-20 text-center bg-white rounded-[2rem] md:rounded-[3rem] border border-gray-100 shadow-sm mx-4 md:mx-0">
                                ...
                                <h3 className="text-xl md:text-2xl font-bold text-brand-primary mb-2">No albums found</h3>
                                ...
                                <button onClick={() => { setSearchQuery(""); setFilterCategory("All"); setJumpYear("All"); setJumpMonth("All"); }} className="mt-6 text-sm font-bold text-amber-600 hover:text-amber-700 underline">
                                    Clear all filters
                                </button>
                            </div>
                        )}
                    ...
                    <section className="max-w-7xl mx-auto px-4 md:px-6 mb-24">
                        <div className="bg-brand-primary rounded-[2rem] md:rounded-[3rem] p-8 md:p-12 lg:p-16 flex flex-col lg:flex-row items-center justify-between gap-8 md:gap-10 relative overflow-hidden shadow-2xl">
                            ...
                            <Smartphone size={150} className="absolute -right-5 -bottom-5 md:-right-10 md:-bottom-10 text-white/5 pointer-events-none md:w-[200px] md:h-[200px]" />

                            <div className="relative z-10 max-w-xl text-center lg:text-left">
                                <h2 className="text-2xl md:text-4xl font-serif font-black text-white mb-3 md:mb-4">Want to see more?</h2>
                                <p className="text-slate-300 leading-relaxed text-sm md:text-lg">
                                    Join our digital community! Follow us across our social platforms for daily updates, behind-the-scenes moments, and hundreds of extra photos.
                                </p>
                            </div>
                            ...
                            <div className="relative z-10 flex flex-col sm:flex-row flex-wrap justify-center lg:justify-end gap-3 md:gap-4 w-full lg:w-auto">
                                ...
                                {/* Instagram */}
                                <a href={CHURCH_INFO.socialMedia.instagram} target="_blank" rel="noreferrer" className="flex flex-1 sm:flex-none items-center justify-center gap-2 md:gap-3 bg-white text-brand-primary px-6 md:px-8 py-3 md:py-4 rounded-xl text-sm md:text-base font-bold hover:bg-[#E1306C] hover:text-white transition-all shadow-lg group">
                                    <InstagramIcon size={18} className="group-hover:scale-110 transition-transform"/> Instagram
                                </a>

                                {/* Facebook */}
                                <a href={CHURCH_INFO.socialMedia.facebook} target="_blank" rel="noreferrer" className="flex flex-1 sm:flex-none items-center justify-center gap-2 md:gap-3 bg-white/10 text-white border border-white/20 px-6 md:px-8 py-3 md:py-4 rounded-xl text-sm md:text-base font-bold hover:bg-[#1877F2] hover:text-white transition-all backdrop-blur-sm group">
                                    <FacebookIcon size={18} className="group-hover:scale-110 transition-transform"/> Facebook
                                </a>

                                {/* Twitter / X */}
                                <a href={CHURCH_INFO.socialMedia.twitter} target="_blank" rel="noreferrer" className="flex flex-1 sm:flex-none w-full sm:w-auto items-center justify-center gap-2 md:gap-3 bg-white text-brand-primary px-6 md:px-8 py-3 md:py-4 rounded-xl text-sm md:text-base font-bold hover:bg-slate-900 hover:text-white transition-all shadow-lg group">
                                    <TwitterIcon size={18} className="group-hover:scale-110 transition-transform"/> X (Twitter)
                                </a>
            ...
            <div className={`fixed bottom-8 left-4 md:left-8 z-40 flex flex-col items-center gap-2 transition-all duration-300 transform ${showTopBtn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10 pointer-events-none"}`}>
                <button onClick={scrollToTop} className="p-2 md:p-2 bg-brand-primary text-white rounded-full shadow-2xl hover:bg-amber-600 hover:-translate-y-1 transition-all flex items-center justify-center">
                    <ChevronUp size={20} />
                </button>
