    ...
    const getServiceStatus = (event: any): ServiceStatus | null => {
        ...
        if (details.sessionsList && details.sessionsList.length > 0) {
            let processedSessions: SessionStatus[] = details.sessionsList.map(sess => {
                ...
                let sStart = new Date(targetDate);
                sStart.setHours(sH, sM, 0, 0);
                let sEnd = new Date(targetDate);
        ...
        <div className="bg-slate-50 min-h-screen pb-24">
            ...
            <div className="max-w-5xl mx-auto px-4 md:px-6">
                <div className="mb-10 text-center md:text-left">
                    <h1 className="text-4xl md:text-5xl font-serif font-black text-brand-primary mb-3">Live Service
                        Tracker</h1>
                ...
                <div className="flex flex-col gap-6">
                    {sortedEvents.map((event) => {
                        ...
                        return (
                            <div key={event.id}
                                 className={`relative overflow-hidden rounded-[2rem] border-2 transition-all duration-500 shadow-sm ${
                                ...
                                <div
                                    className="p-6 md:p-8 flex flex-col md:flex-row items-center md:items-stretch gap-6 md:gap-10">

                                    <div
                                        className="flex flex-col items-center justify-center w-full md:w-64 flex-shrink-0 border-b md:border-b-0 md:border-r border-gray-200/50 pb-6 md:pb-0 md:pr-10 text-center">
                                        {isLive ? (
                                            <div className="animate-pulse">
                                                <div
                                                    className="w-16 h-16 bg-red-100 text-red-600 rounded-full flex items-center justify-center mx-auto mb-3 shadow-inner">
                                                    <PlayCircle size={32}/></div>
                                                <h3 className="text-xl font-black text-red-600 uppercase tracking-widest">Live
                                                    Now</h3>
                                                <p className="text-xs font-bold text-red-400 mt-1">Join us in
                                                    person!</p>
                                            </div>
                                        ) : status.state === 'ENDED' ? (
                                            <div>
                                                <div
                                                    className="w-16 h-16 bg-gray-100 text-gray-400 rounded-full flex items-center justify-center mx-auto mb-3">
                                                    <Clock size={32}/></div>
                                                <h3 className="text-lg font-bold text-gray-400 uppercase tracking-widest">Ended</h3>
                                                <p className="text-xs font-bold text-gray-400 mt-1">Check back for next
                                                    week.</p>
                                            </div>
                                        ) : (
                                            <div>
                                                <div
                                                    className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3 shadow-inner ${isClose ? 'bg-amber-200 text-amber-700' : 'bg-slate-100 text-brand-primary'}`}>
                                                    <Clock size={32}/></div>
                                                <h3 className={`text-xl md:text-2xl font-black ${isClose ? 'text-amber-600' : 'text-brand-primary'}`}>{status.countdown}</h3>
                                                <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-2">{status.targetDate.toLocaleDateString('en-US', {
                                                    weekday: 'long',
                                                    month: 'short',
                                                    day: 'numeric'
                                                })}</p>
                                    ...
                                    <div
                                        className="flex-grow flex flex-col justify-center text-center md:text-left w-full">
                                        <div className="flex flex-wrap justify-center md:justify-start gap-2 mb-3">
                                            <span
                                                className="text-[10px] font-black uppercase tracking-widest text-purple-600 bg-purple-100 px-3 py-1 rounded-full">Every {event.recurrence_rules?.day}</span>
                                            {status.specificTheme && (
                                                <span
                                                    className="text-[10px] font-black uppercase tracking-widest text-white bg-brand-primary px-3 py-1 rounded-full shadow-sm animate-in fade-in">⭐ {status.specificTheme}</span>
                                            )}
                                        </div>
                                        ...
                                        {status.sessions && status.sessions.length > 0 ? (
                                            <div
                                                className="mt-2 bg-slate-50/80 rounded-2xl border border-gray-100 p-4 space-y-2 w-full">
                                                <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3 border-b border-gray-100 pb-2">Schedule
                                                    breakdown</h4>
                                                    ...
                                                    {status.sessions.map((sess, idx) => (
                                                        <div key={idx}
                                                            ...
                                                            <div className="flex flex-col text-left">
                                                                <span
                                                                    className={`font-bold text-sm ${sess.state === 'LIVE' ? 'text-red-700' : 'text-brand-primary'}`}>{sess.name}</span>
                                                                <span
                                                                    className="text-[10px] font-bold text-gray-500">{sess.start_time} {sess.end_time ? `- ${sess.end_time}` : ''}</span>
                                                            </div>
                                                            <div>
                                                                {sess.state === 'LIVE' && <span
                                                                    className="bg-red-600 text-white text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md animate-pulse">Live Now</span>}
                                                                {sess.state === 'ENDED' && <span
                                                                    className="flex items-center gap-1 text-gray-400 text-[10px] font-bold uppercase"><CheckCircle2
                                                                    size={12}/> Ended</span>}
                                                                {sess.state === 'UPCOMING' && <span
                                                                    className="bg-amber-100 text-amber-700 text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md">{sess.countdown}</span>}
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
