...
import { Calendar, MapPin, ArrowRight, Clock, Star, Loader2 } from "lucide-react";
...
export default function NextEvent() {
    ...
    if (loading) {
        return (
            <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex flex-col items-center justify-center min-h-[250px]">
    ...
    if (!nextEvent) {
        return (
            <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex flex-col justify-center min-h-[250px]">
                <div className="w-12 h-12 bg-slate-50 text-slate-400 rounded-full flex items-center justify-center mb-4"><Calendar size={20} /></div>
                <h3 className="font-serif font-bold text-brand-primary text-xl mb-2">No Upcoming Events</h3>
                <p className="text-xs text-gray-500 line-clamp-2 mb-6">There are no special events scheduled at this time. Join us for our regular weekly services!</p>
                <Link href="/events" className="text-[10px] font-bold uppercase tracking-widest text-brand-primary hover:text-amber-600 flex items-center gap-1 mt-auto">View Calendar <ArrowRight size={12}/></Link>
            </div>
        );
    }
    ...
    return (
        <div className={`p-6 rounded-[2rem] border transition-all flex flex-col justify-between min-h-[250px] relative overflow-hidden group ${isToday ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-100 hover:border-amber-200'}`}>

            {/* Background Graphic if the flyer exists */}
            {nextEvent.flyer_url && (
                <div className="absolute inset-0 bg-slate-900 z-0">
                    <img src={nextEvent.flyer_url} alt="Event" className="w-full h-full object-cover opacity-20 group-hover:scale-105 transition-transform duration-700" />
            ...
            <div className="relative z-10 flex-grow flex flex-col">
                <div className="flex justify-between items-start mb-6">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shadow-sm ${nextEvent.flyer_url ? 'text-amber-400 bg-amber-500/20 backdrop-blur-md border border-amber-500/30' : 'bg-purple-500/20 text-purple-600'}`}>
                ...
                <div className="mt-auto">
                    <div className="flex items-center gap-2">
                        {isToday && (
                            <span className="bg-amber-500 text-white text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded shadow-sm animate-pulse">
                                Today
                            </span>
                        )}
                        <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${nextEvent.flyer_url ? 'bg-white/20 text-white border border-white/20' : 'bg-gray-100 text-gray-500'}`}>
                            {nextEvent.category}
                        </span>
                    </div>
