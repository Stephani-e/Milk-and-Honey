...
import { PlayCircle, Film, User, ArrowRight } from "lucide-react";
...
export default function LatestSermon() {
    ...
    return (
        <div className="relative p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between min-h-[250px] overflow-hidden group">
            <div className="absolute inset-0 z-0">
                <img src={getThumbnail(latestSermon)} alt={latestSermon.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
                {latestSermon.clip_url ? (
                    <div className="mb-4 rounded-xl overflow-hidden shadow-2xl border border-white/20 bg-black">
                        <video src={latestSermon.clip_url} controls className="w-full h-32 md:h-40 object-cover" poster={getThumbnail(latestSermon)} />
                    </div>
                ) : (
