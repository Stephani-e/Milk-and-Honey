        ...
        <div className="relative z-[60] w-full bg-brand-primary text-white border-b border-white/10 shadow-md">
            ...
            <div className="flex items-center justify-between px-4 h-10 md:h-12 max-w-7xl mx-auto gap-4">
                ...
                <div className="flex-1 overflow-hidden relative flex items-center h-full">
                    ...
                    <div key={currentAd.id} className="animate-marquee whitespace-nowrap flex gap-20 text-[10px] md:text-xs font-bold tracking-widest uppercase opacity-90">
            ...
            >
                <div className="max-w-5xl mx-auto px-6 flex flex-col md:flex-row gap-6 md:gap-10 items-center relative">
                    ...
                    {/* Media Display */}
                    {currentAd.media_url && (
                        <div className="w-full md:w-2/5 aspect-video bg-slate-800 rounded-2xl overflow-hidden flex-shrink-0 border border-white/10 shadow-xl relative">
                    ...
                    <div className="flex-1 text-center md:text-left space-y-4 w-full">
                        <div className="flex items-center justify-center md:justify-start gap-4 mb-2">
                            <span className="text-amber-400 font-black uppercase tracking-widest text-[10px] bg-amber-400/10 px-3 py-1 rounded-full border border-amber-400/20 block">
                                Featured Update
                            </span>
                            ...
                            {ads.length > 1 && (
                                <div className="flex items-center gap-2 bg-white/5 rounded-full px-2 py-1">
                                    <button onClick={prevAd} className="p-1 hover:text-amber-400 transition-colors"><ChevronLeft size={14}/></button>
                                    <span className="text-[10px] font-bold text-gray-400">{currentIndex + 1} of {ads.length}</span>
                                    <button onClick={nextAd} className="p-1 hover:text-amber-400 transition-colors"><ChevronRight size={14}/></button>
                                </div>
                            )}
                        </div>
